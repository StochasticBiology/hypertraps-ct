...
parallel.fn = function(fork, srcs, dests, times, training.refs) {
  ...
  return(HyperTraPS(train.dests, initialstates = train.srcs, starttimes = train.times, endtimes = train.times, penalty = 1, seed = 1, length = 5, kernel = 3))
}
...
sf = 2
png("plot-tb-predictions-precise.png", width=800*sf, height=300*sf, res=72*sf)
