...
tb.times = as.matrix(read.table("../Data/ng.2878-S2.txt-cooked.txt-datatime.txt"))

HyperTraPS(tb.dests)
HyperTraPS(tb.dests, initialstates=tb.srcs)
HyperTraPS(tb.dests, initialstates=NULL)
HyperTraPS(tb.dests, starttimes = tb.times)
...
parallel.fn = function(fork, srcs, dests, times) {
  ...
  if(fork == 5) { return(HyperTraPS(dests, initialstates = srcs, starttimes = times, seed = 1, length = 4, kernel = 4))}
  if(fork == 6) { return(HyperTraPS(dests, initialstates = srcs, starttimes = times, seed = 2, length = 4, kernel = 4))}
  if(fork == 7) { return(HyperTraPS(dests, initialstates = srcs, starttimes = times, penalty = 1, seed = 1, length = 4, kernel = 4))}
  if(fork == 8) { return(HyperTraPS(dests, initialstates = srcs, starttimes = times, penalty = 1, seed = 2, length = 4, kernel = 4))}
}
...
g.tb.bubbles = ggplot(bdf, aes(x=Time+Expt/10, y=OriginalIndex, size=Probability, color=ExptLabel)) +
  geom_point() + labs(x = "Ordering", y = "Feature", color = "Experiment") +
  theme_light()
g.tb.bubbles

load("tb-8.Rdata")
tb.post.1 = parallelised.runs[[3]]
